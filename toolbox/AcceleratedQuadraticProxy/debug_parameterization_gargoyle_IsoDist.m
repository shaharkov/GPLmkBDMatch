...
%% generate problem
% load mesh
[V, F] = read_obj('data\gargoyle_cut.obj'); 
V=V'; F=F'; F = F(:,[1 3 2]);
...
%% setup solver
solver{1} = OptimSolverAcclQuadProx('AQP', optimProblem, true, true, true); solver{1}.setKappa(100);
solver{2} = OptimSolverAcclQuadProx('QP', optimProblem, false, true, true);
